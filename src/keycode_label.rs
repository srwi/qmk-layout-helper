...
pub fn get_keycode_label(bytes: u16, layer: u8) -> KeycodeLabel {
    get_basic_keycode_label(bytes, layer)
        ...
        .or_else(|| get_advanced_keycode_label(bytes, layer))
        .unwrap_or_else(|| get_hex_keycode_label(bytes, layer))
}

fn get_hex_keycode_label(keycode_bytes: u16, layer: u8) -> KeycodeLabel {
    KeycodeLabel {
...
fn get_advanced_keycode_label(_keycode_bytes: u16, _layer: u8) -> Option<KeycodeLabel> {
    None
}

fn get_layer_keycode_label(keycode_bytes: u16, layer: u8) -> Option<KeycodeLabel> {
    let mut layer = layer;

    let long_name = match keycode_bytes {
        b if QK_TO.contains(&b) => {
            layer = (b - QK_TO.start) as u8;
            format!("TO({})", layer)
        }
        b if QK_MOMENTARY.contains(&b) => {
            layer = (b - QK_MOMENTARY.start) as u8;
            format!("MO({})", layer)
        }
        b if QK_DEF_LAYER.contains(&b) => {
            layer = (b - QK_DEF_LAYER.start) as u8;
            format!("DF({})", layer)
        }
        b if QK_TOGGLE_LAYER.contains(&b) => {
            layer = (b - QK_TOGGLE_LAYER.start) as u8;
            format!("TG({})", layer)
        }
        b if QK_ONE_SHOT_LAYER.contains(&b) => {
            layer = (b - QK_ONE_SHOT_LAYER.start) as u8;
            format!("OSL({})", layer)
        }
        b if QK_LAYER_TAP_TOGGLE.contains(&b) => {
            layer = (b - QK_LAYER_TAP_TOGGLE.start) as u8;
            format!("TT({})", layer)
        }
...
fn get_basic_keycode_label(keycode_bytes: u16, layer: u8) -> Option<KeycodeLabel> {
